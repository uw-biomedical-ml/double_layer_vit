...
from segm.data.dls import DLS_CATS_PATH
...
def main(model_path, input_dir, output_dir, gpu):
    #print(os.path.join(output_dir.replace("/validation",""), "val_filename.npy"))
    ...
    cat_names, cat_colors = dataset_cat_description(DLS_CATS_PATH)
    ...
    #list_seg_pred, list_filename, list_logits_pred = [], [], []
    for i, filename in enumerate(tqdm(list_dir, ncols=80)):
        ...
        pil_seg.convert("RGB").save(output_dir / filename.name)
        pil_blend = Image.blend(pil_im, pil_seg, 0.5).convert("RGB")
        pil_blend.save(output_dir / filename.name.replace(".jpg", "_overlay.jpg"))
